...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    Eigen::Quaterniond Qwb_last(init_Rwb_);            // quaterniond:  from imu measurements
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        Eigen::Vector3d dtheta_half = 0.25 * (imupose.imu_gyro + imupose_last.imu_gyro) * dt;
        ...
        /// 中值积分
        Qwb = Qwb_last * dq;
        Eigen::Vector3d acc_w = 0.5 * (Qwb_last * imupose_last.imu_acc + Qwb * imupose.imu_acc) + gw;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw = Vw + acc_w * dt;
