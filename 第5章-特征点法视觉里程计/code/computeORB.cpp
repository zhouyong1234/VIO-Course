...
// global variables
string first_file = "/home/touchair/下载/视觉SLAM课程/L5/code/1.png";
string second_file = "/home/touchair/下载/视觉SLAM课程/L5/code/2.png";
...
int main(int argc, char **argv) {
  ...
  evaluate_and_call(
      ...
      "compute orb descriptor_1", 1);

  #else


  evaluate_and_call(
      [&]() { computeORBDescMT(first_image, keypoints, descriptors); },
      "compute orb descriptor mt_1", 1);
  ...
  evaluate_and_call([&]() { computeAngle(second_image, keypoints2); },
                    "compute angle_2", 1);

  #else
  

  evaluate_and_call([&]() { computeAngleMT(second_image, keypoints2); },
                    "compute angle mt_2", 1);
  ...
  evaluate_and_call(
      [&]() { computeORBDesc(second_image, keypoints2, descriptors2); },
      "compute orb descriptor_2", 1);

  #else

  evaluate_and_call(
      [&]() { computeORBDescMT(second_image, keypoints2, descriptors2); },
      "compute orb descriptor mt_2", 1);
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints,
  ...
  for (auto &kp : keypoints) {
    ...
    for (int i = 0; i < 256; i++) {
      // START YOUR CODE HERE (~7 lines)
      d[i] = 0;
      double cos_ = cos(kp.angle * pi / 180), sin_ = sin(kp.angle * pi / 180);

      cv::Point2f up_t(cos_ * ORB_pattern[4 * i] - sin_ * ORB_pattern[4 * i + 1], sin_ * ORB_pattern[ 4 * i] + cos_ * ORB_pattern[ 4 * i + 1]);

      cv::Point2f uq_t(cos_ * ORB_pattern[4 * i + 2] - sin_ * ORB_pattern[4 * i + 3], sin_ * ORB_pattern[4 * i + 2] + cos_ * ORB_pattern[4 * i + 3]);

      cv::Point2f up = up_t + kp.pt;
      cv::Point2f uq = uq_t + kp.pt;

      if(up.x < 0 || up.y < 0 || up.x > image.cols || up.y > image.rows || uq.x < 0 || uq.y < 0 || uq.x > image.cols || uq.y > image.rows)
      {
        d.clear();
        break;
      }
